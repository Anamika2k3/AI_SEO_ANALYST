...
export default function Dashboard() {
  ...
  const algorithmUpdates = useMemo(() => [
  ...
  }, [sites, setInsights, setShowInsights]);
  ...
  }, [selectedSite, startDate, endDate, device, activeTab, data]);
  ...
  }, [data, showAlgorithmUpdates, startDate, endDate, algorithmUpdates]);
  ...
  }, [data, showAlgorithmUpdates, createChart]);
  ...
  const fetchData = async (targetTab?: 'queries' | 'pages' | 'country') => {
    ...
    try {
      ...
      console.log(`DEBUG: API request URL: ${apiUrl(`/api/data?${params.toString()}`)}`);
  ...
  const getDailyInsights = async () => {
    ...
    try {
      const response = await apiFetch('/api/insights/daily', {
        method: 'POST',
        headers: {
  ...
  const getQueryInsights = async () => {
    ...
    try {
      const response = await apiFetch('/api/insights/queries', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 mt-2">Monitor search performance, intent, and opportunity signals in real time.</p>
          {performanceData && (
